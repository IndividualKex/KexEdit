...
def validate_facing(filepath):
    ...
    for i, sn in enumerate(nodes):
        ...
        has_direction = abs(anchor.direction[0]) > 0.01 or abs(anchor.direction[1]) > 0.01 or abs(anchor.direction[2]) > 0.01
        has_position = abs(anchor.heart_position[0]) > 0.01 or abs(anchor.heart_position[1]) > 0.01 or abs(anchor.heart_position[2]) > 0.01

        if has_direction or has_position:
            print(f"  Position: ({anchor.heart_position[0]:.2f}, {anchor.heart_position[1]:.2f}, {anchor.heart_position[2]:.2f})")
            print(f"  Direction: ({anchor.direction[0]:.3f}, {anchor.direction[1]:.3f}, {anchor.direction[2]:.3f})")

        if sn.node.type == NodeType.Reverse:
            print(f"  [REVERSE NODE] Expected facing to flip direction")
    ...
    if all_forward:
        ...
        print(f"Nodes with facing != 1:")
        for sn in reverse_nodes:
            print(f"  Node {sn.node.id} ({get_node_type_name(sn.node.type)}): facing={sn.anchor.facing}")
