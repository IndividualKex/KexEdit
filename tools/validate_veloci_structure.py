...
def validate_file_structure(filepath):
    ...
    duplicates = {nid: indices for nid, indices in node_id_map.items() if len(indices) > 1}
    if duplicates:
        for nid, indices in duplicates.items():
            print(f"  [ERROR] Node ID {nid} appears {len(indices)} times at indices {indices}")
            for idx in indices:
                sn = nodes[idx]
                print(f"    [{idx}] Type: {get_node_type_name(sn.node.type)}, Pos: {sn.node.position}")
    ...
    if dup_ports:
        for pid, locs in dup_ports.items():
            ...
            for direction, nidx, pidx in locs:
                sn = nodes[nidx]
                print(f"    Node {sn.node.id} ({get_node_type_name(sn.node.type)}) {direction}[{pidx}]")
    ...
    dup_edges = {eid: indices for eid, indices in edge_id_map.items() if len(indices) > 1}
    if dup_edges:
        for eid, indices in dup_edges.items():
            print(f"  [ERROR] Edge ID {eid} appears {len(indices)} times at indices {indices}")
    ...
    bridges = [(idx, sn) for idx, sn in enumerate(nodes) if sn.node.type == NodeType.Bridge]
    for idx, bridge in bridges:
        ...
        anchor_inputs = [p for p in bridge.input_ports if p.port.type == 0]  # PortType.Anchor
            ...
            if target_edges:
                for e in target_edges:
                    ...
                    if locs:
                        ...
                        print(f"      Connected to: Node {src_node.node.id} ({get_node_type_name(src_node.node.type)})")
            else:
                print(f"      [WARNING] No edges connect to Target port")
                print(f"      Anchor data: pos={bridge.anchor.heart_position[:2]}, dir={bridge.anchor.direction[:2]}")
                has_data = (abs(bridge.anchor.heart_position[0]) > 0.01 or
                           abs(bridge.anchor.heart_position[1]) > 0.01 or
                           abs(bridge.anchor.direction[0]) > 0.01 or
                           abs(bridge.anchor.direction[1]) > 0.01)
                if has_data:
                    print(f"      [INFO] Will create synthetic Anchor at {bridge.node.position[0] - 100}, {bridge.node.position[1] + 50}")
        else:
            print(f"    [WARNING] Bridge has only {len(anchor_inputs)} Anchor inputs (expected 2)")
