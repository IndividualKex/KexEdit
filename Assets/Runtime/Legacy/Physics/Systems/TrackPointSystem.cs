    ...
    public partial struct TrackPointSystem : ISystem {
        ...
        private struct CalculateCountJob : IJobParallelFor {
            ...
            public void Execute(int index) {
                ...
                float trackLength = points[endIndex].HeartArc() - points[startIndex].HeartArc();
                if (trackLength <= 0f) {
                    CountMap.TryAdd(entity, 2);
                    return;
                }
            ...
            private (int startIndex, int endIndex) GetPointRange(DynamicBuffer<CorePointBuffer> points, Segment segment) {
                if (points.Length < 2) {
                    return (0, 0);
                }
        ...
        private partial struct BuildJob : IJobEntity {
            ...
            public void Execute(Entity entity, ref TrackHash trackHash, in SectionReference section, in Segment segment) {
                ...
                var last = points[endIndex];
                float startLength = first.HeartArc();
                float endLength = last.HeartArc();
                ...
                for (int i = 0; i <= desiredCount; i++) {
                    ...
                    while (currentSegment < endIndex - 1 && points[currentSegment + 1].HeartArc() < targetLength) {
                        currentSegment++;
                    }

                    var p0 = points[currentSegment];
                    var p1 = points[currentSegment + 1];

                    float segmentT = math.saturate((targetLength - p0.HeartArc()) / (p1.HeartArc() - p0.HeartArc()));

                    float3 position = math.lerp(p0.GetSpinePosition(p0.HeartOffset()), p1.GetSpinePosition(p1.HeartOffset()), segmentT);
                    float3 direction = math.lerp(p0.GetSpineDirection(p0.HeartOffset()), p1.GetSpineDirection(p1.HeartOffset()), segmentT);
                    float3 lateral = math.lerp(p0.GetSpineLateral(p0.HeartOffset()), p1.GetSpineLateral(p1.HeartOffset()), segmentT);
                    float3 normal = -math.normalize(math.cross(direction, lateral));
                    float distance = targetLength - startLength;
                    float heart = math.lerp(p0.HeartOffset(), p1.HeartOffset(), segmentT);
            ...
            private static (int startIndex, int endIndex) GetPointRange(DynamicBuffer<CorePointBuffer> points, Segment segment) {
                if (points.Length < 2) {
                    return (0, 0);
                }
            ...
            private float GetVisualizationValue(Entity section, int index, in CorePointBuffer p0, in CorePointBuffer p1, float t) {
                float value0, value1;
