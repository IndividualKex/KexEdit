...
namespace Tests {
    public static class GeometricSectionEntityBuilder {
        public static Entity Create(EntityManager em, GoldSection section) {
            ...
            em.AddBuffer<PitchSpeedKeyframe>(entity);
            em.AddBuffer<YawSpeedKeyframe>(entity);
            ...
            em.SetComponentData(entity, new Steering { Value = section.inputs.steering });
        ...
        private static void PopulateKeyframes(EntityManager em, Entity entity, GoldKeyframes kf) {
            ...
            if (kf.pitchSpeed != null) {
                var buffer = em.GetBuffer<PitchSpeedKeyframe>(entity);
                foreach (var k in kf.pitchSpeed) buffer.Add(ToKeyframe(k));
            }
            if (kf.yawSpeed != null) {
                var buffer = em.GetBuffer<YawSpeedKeyframe>(entity);
                foreach (var k in kf.yawSpeed) buffer.Add(ToKeyframe(k));
            }
