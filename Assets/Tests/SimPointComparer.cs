    ...
    public static class SimPointComparer {
        ...
        private static void AssertMatchesGoldInternal(
            ...
            for (int i = 0; i < expected.Count; i++) {
                float tolerance = BASE_TOLERANCE + TOLERANCE_PER_STEP * (cumulativeOffset + i);
        ...
        private static IEnumerable<int> GetSampleIndices(int count) {
            ...
            for (int i = 0; i < math.min(BOUNDARY_COUNT, count); i++) {
                if (yielded.Add(i)) yield return i;
            }

            for (int i = math.max(0, count - BOUNDARY_COUNT); i < count; i++) {
                if (yielded.Add(i)) yield return i;
            }

            for (int i = SAMPLE_INTERVAL; i < count - BOUNDARY_COUNT; i += SAMPLE_INTERVAL) {
                if (yielded.Add(i)) yield return i;
            }
        }
