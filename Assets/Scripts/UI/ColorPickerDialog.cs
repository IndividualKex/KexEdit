...
namespace KexEdit.UI {
    public class ColorPickerDialog : VisualElement {
        ...
        public ColorPickerDialog(Action onClose) {
            _onClose = onClose;
            _currentTrackStyle = TrackStylePreferences.CurrentTrackMesh;
        ...
        private void CreateColorField(int colorIndex, Color defaultColor) {
            ...
            var currentColor = ColorPreferences.GetColor(_currentTrackStyle, colorIndex, defaultColor);
        ...
        private void OnColorFieldChanged(ColorFieldRow colorRow) {
            ...
            ColorPreferences.SetColor(_currentTrackStyle, colorRow.Index, newColor);

            colorRow.RField.SetValueWithoutNotify(r);
            colorRow.GField.SetValueWithoutNotify(g);
            colorRow.BField.SetValueWithoutNotify(b);

            TriggerTrackMeshReload();
        }

        private void TriggerTrackMeshReload() {
            try {
                ...
                Debug.LogError($"Failed to trigger track mesh reload: {ex.Message}");
            }
        }

        private void ResetToDefaults() {
            ColorPreferences.ResetAllColors(_currentTrackStyle);
