...
namespace KexEdit.UI {
    public static class TrainStyleConfigManager {
        ...
        public static void LoadConfig(string configFileName) {
            ...
            try {
                Preferences.CurrentTrainSTyle = configFileName;

                var world = World.DefaultGameObjectInjectionWorld;
                if (world?.EntityManager != null) {
                    var entityManager = world.EntityManager;
                    var loadEvent = entityManager.CreateEntity();
                    entityManager.AddComponentData(loadEvent, new LoadTrainStyleConfigEvent { Value = true });
                }
                else {
                    Debug.LogWarning("Default world or EntityManager not available for train style loading");
                }
            }
            catch (System.Exception e) {
                Debug.LogError($"Failed to load train style config {configFileName}: {e.Message}");
            }
        }
